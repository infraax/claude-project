 ...
 *   - Key context sections (Critical Facts, Last Session Summary, Open Questions)
 *   - Agent roster
 *   - Services
 *   - Enabled automations
 *   - Device map
...
export function generateClaudeMd(options: GenerateOptions = {}): string {
  ...
  // v5: context comes from get_context() MCP tool / context.json
  const contextPath = path.join(paths.runtimeDir, 'context.json');
  if (fs.existsSync(contextPath)) {
    try {
      const ctx = JSON.parse(fs.readFileSync(contextPath, 'utf-8'));
      wakeupCriticalFacts = (ctx.critical_facts ?? []).join('\n');
      wakeupLastSession   = ctx.summary ?? '';
      wakeupOpenQuestions = (ctx.next_steps ?? []).join('\n');
    } catch { /* non-fatal */ }
  }
  ...
  // devices and shared_paths removed in v5
  ...
  lines.push(
    ...
    `---`,
    ``,
    `*Auto-generated by [claude-project](https://github.com/infraax/claude-project) v5.*`,
